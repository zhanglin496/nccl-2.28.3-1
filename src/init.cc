...
//baozheng meige xianchen de zhan kongjian buxiao yu 8 MB
...
static void initOnceFunc() {
  ...
  //支持CPU直接访问GPU内存
  initGdrCopy();
   //为 NCCL 的 Bootstrap 阶段 选定 一张本机监听网卡 + 地址，并登记到全局变量
  // Always initialize bootstrap network
  NCCLCHECKGOTO(bootstrapNetInit(), initResult, exit);
   //在 首次规约前 把 ncclRedOp_t 的 数值→字符串 表注册给 NVTX，
  //使得后续所有 NCCL reduction 的抓痕能 直接显示“Sum/Prod/Max”等可读枚举名，
  //而非裸数字，对性能零影响，对调试极友好。
...
ncclResult_t ncclGetUniqueId(ncclUniqueId* out) {
  //生成一个ip地址保存到bootstrapNetIfAddr
  NCCLCHECK(ncclInit());
  NCCLCHECK(PtrCheck(out, "GetUniqueId", "out"));
  struct ncclBootstrapHandle handle;
  //获取一个ip监听地址和magic到handle
  ...
  //根据handle选择的ip地址+端口+magic的值hash算出一个8字节的ID
...
static ncclResult_t commAlloc(struct ncclComm* comm, struct ncclComm* parent, int ndev, int rank) {
  ...
  //ncclNetInit() 按顺序尝试加载插件，一旦某个插件分配成功，循环立即 break，后面所有插件都不再尝试
  //设置comm中的ncclNet和ncclCollNet，ncclCollNet可能为NULL
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  // AllGather1 - begin
  //分配空间，存储所有rank的peerinfo信息
  //+1 for Extra rank to represent CollNet root
...
//先假设支持cumem
  ...
  for (int i = 0; i < nranks; i++) {
    ...
    //检查节点的数量，host hash不同，表示不在同一个物理机上，统计节点的数量
    if (comm->peerInfo[i].hostHash != comm->peerInfo[rank].hostHash)
        nNodes++;

    //只要有一个rank不支持cumem，则不使用cumem
    if (!comm->peerInfo[i].cuMemSupport) 
        comm->cuMemSupport = 0;
    
    //不同的通信域内，可以绑定到同一个gpu
    //同一个通信域内，当前 Rank（rank）与另一个 Rank（i）在同一台物理机上，且绑定到了同一块 CUDA GPU​​。
    //比如Rank 0 和 Rank 1 都绑定 GPU 0——​​非法​​，NCCL 会直接报错。
    //同一个通信域内，rank和gpu是一对一的绑定关系，不同 Rank 必须绑定不同物理 GPU
  ...
  //// 获取系统的拓扑信息，并存储在comm的topo成员中  ，不dump xml文件
...
// 在已获取的拓扑中，计算GPU和NIC之间的路径 
 ...
 // 初始化拓扑搜索
  ...
  //下面开始统计带宽和要使用maxChannels通道信息，并填入到ncclTopoGraph中
  // Get rings and trees
  //先清0
...
//只检查当前节点的所有rank gpu是否支持p2p
  ...
  //所有node中最大的localrank数量
    int nLocals = comm->maxLocalRanks;
    struct ncclNodeRanks* nodeRanks = comm->nodeRanks;
    bool flat = false;
    //检查所有node上的localrank数量是否相同
    for (int node = 0; node < nNodes; node++) {
        //节点间rank数量不相同
        //如节点0有4GPU，节点1有2GPU，退化为扁平化处理。
...
//ncclParamRuntimeConnect默认为1
  ...
  //这个调用确保了在同一个节点内的所有GPU ranks都执行到了这个同步点，
  //也就是initTransportsRank操作都执行完毕了
  //就是一个序列点同步，只不过是用通信的方式来实现同步，而不是用锁实现
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  if (job->parent) {
    ...
    //根据ncclUniqueId生成一个8字节的id号
    comm->commHash = commIdHash = getHash(job->commId->internal, NCCL_UNIQUE_ID_BYTES);

    timers[TIMER_INIT_ALLOC] = clockNano();
    
    //继续初始化comm结构体。内部调用ncclNetInit中初始化网络插件
    //函数实现会加载外部库和2个最重要的内部网络插件，ncclNetIb和ncclNetSocket
...
static ncclResult_t ncclCommInitRankDev(ncclComm_t* newcomm, int nranks, int nId, ncclUniqueId* commId, int myrank, int cudaDev, ncclConfig_t *config, const char funcName[]) {
  ...
  //初始化环境变量，这个了root rank已经调用过了不再执行
  //其他rank会执行，获取地址到到bootstrapNetIfAddr
  ...
  *newcomm = comm;
  ...
  if (commIdEnv && myrank == 0) {
    ...
    //如果设置了NCCL_COMM_ID，强制设置nld为1，并且只能使用rank0来创建root线程
    if (nId > 1) {
...
ncclResult_t ncclCommInitRank(ncclComm_t* newcomm, int nranks, ncclUniqueId commId, int myrank) {
  ...
  //获取当前进程绑定的gpu设备，在调用ncclCommInitRank之前，
  //需要调用  cudaSetDevice(localRank)绑定到那个gpu设备
  CUDACHECK(cudaGetDevice(&cudaDev));

   //默认只有一个root
  NCCLCHECK(ncclCommInitRankDev(newcomm, nranks, /* root数量*/1, &commId, myrank, cudaDev, &config, __func__));
