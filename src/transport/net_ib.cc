...
ncclResult_t ncclIbInit(void** ctx, uint64_t commId, ncclNetCommConfig_t* config, ncclDebugLogger_t logFunction, ncclProfilerCallback_t profFunction) {
  ...
  //加载ibv函数指针到ibvSymbols
  if(wrap_ibv_symbols() != ncclSuccess) { 
    return ncclInternalError; 
  }

  //加载mlx5dev函数指针到mlx5dvSymbols
    ...
    if (ncclNIbDevs == -1) {
      ...
      if (searchExact) 
        userIbEnv++;
            ...
            //解析userIbEnv到userIfs
            //比如：userIbEnv=mlx5_0:1,mlx5_1:1 Use ports 1 of cards mlx5_0 and mlx5_1.
