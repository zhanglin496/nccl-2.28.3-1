...
static ncclResult_t getPciPath(const char* busId, char** path) {
   //用busid替换path中的0000:00和0000:00:00.0
  char busPath[] = "/sys/class/pci_bus/0000:00/../../0000:00:00.0";
  //0000:00:00.0为12个字符
  //这里只拷贝pci号的0000:00前7个字符
  //假设busid是0000:0b:00.0
  //busPath为：/sys/class/pci_bus/0000:0b
  memcpylower(busPath+sizeof("/sys/class/pci_bus/")-1, busId, BUSID_REDUCED_SIZE-1);
  //这里拷贝完整的pci号
  //busPath变为：/sys/class/pci_bus/0000:0b/../../0000:0b:00.0
  memcpylower(busPath+sizeof("/sys/class/pci_bus/0000:00/../../")-1, busId, BUSID_SIZE-1);
  //这里是个软连接，获取实际路径
  //这个函数本质就是获取pci对应的sys实际路径
  //例如：
  // realpath /sys/class/pci_bus/0000\:0b/../../0000:0b:00.0
  // /sys/devices/pci0000:0a/0000:0a:01.1/0000:0b:00.0
...
ncclResult_t ncclTopoGetXmlFromSys(struct ncclXmlNode* pciNode, struct ncclXml* xml) {
  ...
  //假设获取的path为/sys/devices/pci0000:0a/0000:0a:01.1/0000:0b:00.0
  //0000:0b:00.0代表当前的gpu设备pci号
  getPciPath(busId, &path);
  ncclDebugNoWarn = 0;

  if (path) {
    //获取/sys/devices/pci0000:0a/0000:0a:01.1/0000:0b:00.0/class的值
    //同时写入到pciNode中
  ...
  if (parent == NULL) {
    if (path) {
      ...
      int slashCount = 0;
      int parentOffset;
      //扫描路径/sys/devices/pci0000:0a/0000:0a:01.1/0000:0b:00.0
...
ncclResult_t ncclTopoFillGpu(struct ncclXml* xml, const char* busId, struct ncclXmlNode** gpuNode) {
  ...
  //nvml 是 NVIDIA Management Library 的缩写，即 NVIDIA GPU 管理库。
